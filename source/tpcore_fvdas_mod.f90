...
!
  ...
  REAL*8, ALLOCATABLE       :: fy (:, :, :, :)           ! one more for edges
...
!BOC
  ...
  END SUBROUTINE Exit_Tpcore
     ...
     subroutine tp_save(file_name, var_name, data_con)
        use netcdf
        include "CMN_SIZE"
        character(len=*)     :: file_name
        character(len=*)     :: var_name
        real*8               :: data_con(IIPAR, JJPAR, LLPAR)
        integer :: ncid, varid, dimid_x, dimid_y, dimid_z, dimids(3), res
        print *, file_name

        res = nf90_create(trim(file_name), NF90_CLOBBER, ncid)

        res = nf90_def_dim(ncid, "lon", IIPAR, dimid_x)
        res = nf90_def_dim(ncid, "lat", JJPAR, dimid_y)
        res = nf90_def_dim(ncid, "alt", LLPAR, dimid_z)

        dimids = (/dimid_x, dimid_y, dimid_z/)

        res = nf90_def_var(ncid, trim(var_name), NF90_REAL, dimids, varid)

        res = nf90_enddef(ncid)

        res = nf90_put_var(ncid, varid, data_con)

        res = nf90_close(ncid)
        print *, 'complete'
    end subroutine

    subroutine check_neg_val(data_con, location)
      include "CMN_SIZE"
      real*8               :: data_con(IIPAR, JJPAR, LLPAR)
      integer              :: i, j, l
      ...
      do i = 1, IIPAR
        do j = 1, JJPAR
          do l = 1, LLPAR
            if (data_con(i, j, l) < 0) then
              print *, 'NOW IS CHECK  ', location
              print *, i, ' ', j, ' ', l
              stop
            end if
          end do
    ...
    subroutine check_neg_val_2d(data_con, location)
      include "CMN_SIZE"
      real*8               :: data_con(IIPAR, JJPAR)
      integer              :: i, j
      character(len=*)     :: location

      do i = 1, IIPAR
        do j = 1, JJPAR
            if (data_con(i, j) < 0) then
              print *, 'NOW IS CHECK  ', location
              print *, i, ' ', j
              stop
            end if
    ...
    subroutine check_all(data_con, data1, data2,  location)
      include "CMN_SIZE"
      real*8               :: data_con(IIPAR, JJPAR)
      real*8               :: data1(IIPAR, JJPAR)
      real*8               :: data2(IIPAR, JJPAR)
      integer              :: i, j
      character(len=*)     :: location
      ...
      do i = 1, IIPAR
        do j = 1, JJPAR
            if (data_con(i, j) < 0) then
              print *, 'NOW IS CHECK  ', location
              print *, i, ' ', j
              print *, 'data_con', data_con(i, j)
              print *, 'data1', data1(i, j)
              print *, 'data2', data2(i, j)
              stop
            end if
        end do
      end do
...
!
  SUBROUTINE Tpcore_FvDas( dt,       ae,       IM,      JM,     KM,       &
                           ...
                           MASSFLNS, MASSFLUP, AREA_M2,   ND24,           &
                           ND25,     ND26,     FILL )
!
! !USES:
!
...
! !DEFINED PARAMETERS:
    ...
    INTEGER, SAVE      :: name_val
    character(len=40)   :: save_file_name
!
! !LOCAL VARIABLES:
!
    INTEGER            :: rj2m1
    ...
    name_val = name_val + 1
    !write(save_file_name, "('/data/ese-zhengzy/forward/dp2_', I2.2,'.nc')")  name_val
...
!$OMP PRIVATE( IK, IQ )
  ...
  ! =================
    ...
    if ( ik == 72 ) then
      print *, 'Courant   ', maxval(cx(:,:,72)), '   y   ', maxval(cy(:,:,72))
    end if
...
!$OMP END PARALLEL DO
  ...
  ! ==============
    ...
    !delp2(:, :, 1) = ps2
    !call tp_save(save_file_name, 'test', delp2(:, :, 72:1:-1))
...
!--------------------------------------------------------
    !print *, 'sum  q', sum(q)
    !dq1(:, :, :) = q(:,:,:,1) * delp1(:,:,:)
    !print *, 'sum  dq1', sum(dq1)
    !print *, 'min q', minval(q)
    !print *, 'min delp1', minval(delp1)
    !print *, 'min dq1', minval(dq1)
    !write(save_file_name, "('/data/ese-zhengzy/forward/ua_', I2.2,'.nc')") name_val
    !call tp_save(save_file_name, 'test', ua(:, :, 72:1:-1))
    !write(save_file_name, "('/data/ese-zhengzy/forward/va_', I2.2,'.nc')") name_val
    !call tp_save(save_file_name, 'test', va(:, :, 72:1:-1))

    !call check_neg_val(dq1, 'dq1 before loop')
    do iq = 1, 2
       do ik = 1, km
       !.sds.. convert to "mass"
       !print *, 'Now layer is ik'
       dq1(:,:,ik) = q(:,:,ik,iq) * delp1(:,:,ik)


       !print *, 'Calc_Advec_Cross_Terms'
     ! ===========================
     ...
     ! ==============
       ...
       !           - use in fzppm
       !print *, ik
       !print *, 'Now dq1 is ', dq1(1102, 52, ik)
       !print *, 'Now q is ', q(1102, 52, ik, iq)
       !print *, 'Now qqv is ', qqv(1102, 52)
       !call check_neg_val_2d(qqu, 'qqu')
       !call check_neg_val_2d(qqv, 'qqv')
       !call check_neg_val_2d(ady, 'adx')
       !call check_neg_val_2d(adx, 'adx')
       !call check_neg_val_2d(adx, 'adx')
       !print *, 'check q is ', q(1103, 52, ik, iq)
       q(:,:,ik,iq) = q(:,:,ik,iq) + ady + adx
       !print *, ik
       !print *, 'ua ', maxval(ua(:, :, ik))
       !print *, 'va ', maxval(va(:, :, ik))
       !call check_all(q(:, :, ik, iq), adx, ady, 'q after adx ady')
       !print *, 'check qqv is ', qqv(1103, 52)
       !print *, 'check qqu is ', qqu(1103, 52)
       !print *, 'check adx is ', adx(1103, 52)
       !print *, 'check ady is ', ady(1103, 52)
       !call check_neg_val_2d(q(:, :, ik, iq), 'q after adx ady')
       !dq1(:,:,ik) = q(:,:,ik,iq) * delp1(:,:,ik)
       !print *, dq1(1, 593, ik)
       !print *, 'check qqv'
       !print *, qqv(1152, 593), ' ', qqv(1, 593), ' ', qqv(2, 593)
       !print *, xmass(1152, 593, ik), ' ', xmass(1, 593, ik), ' ', xmass(2, 593, ik)
       !print *, q(1152, 593, ik, iq), ' ', q(1, 593, ik, iq), ' ', q(2, 593,ik, iq)
       !q(:,:,ik,iq) = q(:,:,ik,iq) + ady + adx
       !print *, q(1152, 593, ik, iq), ' ', q(1, 593, ik, iq), ' ', q(2, 593,ik, iq)
       call Xtp  &
     ! ========
            ...
            j1p, j2p, im, 1, jm, 1, im, 1, jm, &
            1, im, 1, jm, IORD)
       !call check_neg_val(dq1, 'dq1 after Xtp')
       !.sds notes on output arrays
       ...
       !.sds
       !print *, 'Ytp'
       !call check_neg_val_2d(dq1(:, :, ik), 'dq1 after xtp')
       !print *, dq1(1, 593, ik)
     ! ========
       call Ytp  &
     ! ========
       ...
       !call check_neg_val(dq1, 'dq1 after Ytp')
       !.sds notes on output arrays
       ...
       !.sds
        !print *, dq1(1, 593, ik)
        !call check_neg_val_2d(dq1(:, :, ik), 'dq1 after Ytp')
        end do
       !print *, 'dq1  1',  sum(dq1)
       !call check_neg_val_2d(dq1(:, :, ik), 'after loop dq1')
       !print *, 'q2.5     ', sum(dq1(:, :, 72))
       qtemp(:,:,:,iq) = q(:,:,:,iq)
       !qtemp(:,:,:,iq) = 1
     ...
     ! ==========
       ...
       !print *, 'dq1  2',  sum(dq1)
       !call check_neg_val(dq1, 'after Fzppm')
       !print *, 'sum q val  ', minval(dq1(:, :, 72))
       ...
       !print *, 'dq1   3', sum(dq1)
       !call check_neg_val(dq1, 'after Qckxyz dq1')
       !write(save_file_name, "('/data/ese-zhengzy/forward/dq1_', I2.2,'.nc')") name_val
       !call tp_save(save_file_name, 'test', dq1(:, :, 72:1:-1))
    ...
    DO iq=1,nq
      ...
      DO I = 1, 1152
         ...
         MASSFLEW(I,J,72,IQ) = FX(I,J,K,IQ)
...
!   ====
       ...
       !$OMP PARALLEL DO        &
       !$OMP DEFAULT( SHARED   )&
       !$OMP PRIVATE( il, ij, iu, riu, ru, ril, rij, jv )
...
!             From above.
!             -----------
                !print *, 'negative val   ', dq1(il, ij, ik)
                qup =  dq1(il,ij,ik-1)
                qly = -dq1(il,ij,ik)
                dup =  Min (qly, qup)
...
!   =============
       ...
       !       ----------
          ...
          !$OMP PARALLEL DO        &
          !$OMP DEFAULT( SHARED   )&
          !$OMP PRIVATE( il, ij, iu, riu, ru, rdiff, ril )
...
!   ==================
          ...
          !$OMP PARALLEL DO        &
          !$OMP DEFAULT( SHARED   )&
          !$OMP PRIVATE( il, ij, iu, riu, ru, a1, b1, c1 )
          ...
          do ij = j1p, j2p
             ...
             if ((ij <= js) .or. (ij >= jn)) then
                ...
                do il = i1, i2
                   ...
                   adx(il,ij) = (ru * ((a1 * ru) + b1)) + c1
                   !if ( il == 1 .and. ij == 114 ) then
                   !  print *, 'qtem ', qtmp(iu+1,ij), ' ', &
                   !  qtmp(iu,ij), ' ', qtmp(iu-1,ij)
                   !  print *, 'ua ', ua(il, ij)
                   !  print *, 'adx ', adx(il, ij)
                     !stop
...
!   ==================
          !$OMP PARALLEL DO        &
          !$OMP DEFAULT( SHARED   )&
          !$OMP PRIVATE( il, ij, jv, rjv, rv, a1, b1, c1 )
          do ij = j1p-1, j2p+1
             do il = i1, i2
!c?
                jv  = Nint (va(il,ij))
                ...
                !if ( il == 1 .and. ij == 114 ) then
                !     print *, 'qquwk ', qquwk(il,ij+1), ' ', &
                !     qquwk(il,ij), ' ', qquwk(il,ij-1)
                !     print *, 'va ', va(il, ij)
                !     print *, 'ady ', ady(il, ij)
                     !stop
                !end if
...
!   ==============
       !!$OMP PARALLEL DO        &
       !!$OMP DEFAULT( SHARED   )&
       !!$OMP PRIVATE( ij, il, ril, iu, ic, ric, ix )
       do ij = j1p, j2p
!      =================
...
!           ------------------------------------------------------
             ...
             if ((IORD == 1) .or.  &
                ...
                if ((IORD == 2) .or.  &
                   ...
                   !if ( fx(il, ij) * Sign (1.0d0, rc) < 0 ) then
                   end do
...
!   ----------------
    ...
    r24 = 1.0d0 / 24.0d0
      ...
      !$OMP PARALLEL DO        &
      !$OMP DEFAULT( SHARED   )&
      !$OMP PRIVATE( ij, il, tmp, pmax, pmin )
...
!   -------------------
    ...
    !$OMP PARALLEL DO        &
    !$OMP DEFAULT( SHARED   )&
    !$OMP PRIVATE( ik, ij, il, c0, c1, c2, tmp, qmax, qmin )
    do ik = k1p1, k2m1
...
!     ---------------------------
       !$OMP PARALLEL DO        &
       !$OMP DEFAULT( SHARED  )&
       !$OMP PRIVATE( il, fac1, fac2, aa, bb )
       do il = i1, i2
...
!         -------------------------------
          ...
          !$OMP PARALLEL DO        &
          !$OMP DEFAULT( SHARED  )&
          !$OMP PRIVATE( ik, il, qmp, lac, qmin, qmax )
